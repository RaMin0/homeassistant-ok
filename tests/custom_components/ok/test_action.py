...
async def test_async_call_ok_api_translates_known_command_errors() -> None:
    async def call() -> NoReturn:
        raise OkCommandError(
            "OK command failed: featureDisabled",
            error_description="featureDisabled",
        )
    ...
    assert exc_info.value.translation_domain == "ok"
    assert exc_info.value.translation_key == "command_feature_disabled"
    assert exc_info.value.translation_placeholders in (None, {})
...
def test_active_charging_token_prefers_charging_token() -> None:
    assert (
        active_charging_token({"chargingToken": "charging", "firestoreToken": "firestore"})
        == "charging"
    )
    assert active_charging_token({"firestoreToken": "firestore"}) is None
    assert active_charging_token({}) is None


def test_charging_command_tokens_extracts_action_and_realtime_tokens() -> None:
    assert charging_command_tokens(
        {"chargingToken": "charging", "firestoreToken": "firestore"}
    ) == ("charging", "firestore")
    assert charging_command_tokens({"chargingToken": "", "firestoreToken": 1}) == (None, None)
    assert charging_command_tokens(None) == (None, None)
...
def test_validate_command_response_accepts_success_and_rejects_failures() -> None:
    ...
    with pytest.raises(HomeAssistantError) as exc_info:
        validate_command_response({"errordescription": "UpdateBeforeAction"})

    assert exc_info.value.translation_key == "command_update_before_action"
